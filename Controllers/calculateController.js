...
const geocoder = require('node-geocoder')({ provider: 'openstreetmap', timeout: 10000, formatter: null });
...
exports.calculateChart = async (req, res) => {
  ...
  try {
    ...
    const meta = await Meta.create(
      { place, latitude, longitude, local_time: dtLocal.toDate(), utc_time: dtUtc.toDate() },
      { transaction: t }
    );
    ...
    // Calculate Ketu from Rahu and add to planets array
    ...
    return res.json({
      ...
      planets, // includes Ketu
      lagna,   // includes navamsa
      meta_id: meta.id
    });
  } catch (err) {
...
exports.getChart = async (req, res) => {
  res.status(200).json({ message: "getChart endpoint not implemented yet" });
};

exports.listCharts = async (req, res) => {
  res.status(200).json({ message: "listCharts endpoint not implemented yet" });
};

exports.deleteChart = async (req, res) => {
  res.status(200).json({ message: "deleteChart endpoint not implemented yet" });
};
